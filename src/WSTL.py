"""
WSTL computation graph to capture preferences in the data.
Implementation is inspired from STLCG code base.
See: https://github.com/StanfordASL/stlcg

new version enables WSTL computation for multiple weight
valuations in one shot.
...
class Maxish(torch.nn.Module):
    """
    Defines max and softmax function. Softmax scales with "scale" input.
    If scale = -1 it uses max function.
    """
    ...
    def forward(self, x, scale, axis):
        if isinstance(x, Expression):
            ...
            x = x.value
        if scale > 0:
            return torch.logsumexp(x * scale, dim=axis, keepdim=True) / scale
        else:
            return torch.max(x, axis=axis, keepdim=True)[0]


class Minish(torch.nn.Module):
    """
    Defines min and softmin function. Softmin scales with "scale" input.
    If scale = -1 it uses min function.
    """
    ...
    def forward(self, x, scale, axis):
        ...
        if scale > 0:
            ...
            return torch.min(x, axis=axis, keepdim=True)[0]
...
class WSTL_Formula(torch.nn.Module):
    ...
    def robustness(self, inputs, scale=-1, t=0):
        """
        Returns WSTL robustness value at t=0, by default.
    ...
    def set_weights(self, inputs, w_range, no_samples, random=False, seed=None, **kwargs):
        ...
        If random = True, it initializes weights uniformly random between given
        range.
        no_samples: number of weight valuation samples to be set
        (it can work with random=false, but does not make sense.)"""
        if seed is not None:
...
class Temporal_Operator(WSTL_Formula):
    ...
    Note that Until is defined separately.
    """
